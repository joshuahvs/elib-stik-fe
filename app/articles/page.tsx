...
function extensionLabel(statusRaw: any): string | null {
...
export default function AdminBorrowedBookRequestsPage() {
  ...
  async function approveExtension(row: AdminLoanRow) {
    if (!token) return;
    if (!row.akhir_perpanjangan) return;

    const prevStatusPerpanjangan = row.status_perpanjangan;
    const prevAkhir = row.akhir_peminjaman;
    const nextAkhir = row.akhir_perpanjangan;
      ...
      setRows((current) =>
        current.map((r) =>
          r.id === row.id
            ? {
                ...r,
                status_perpanjangan: prevStatusPerpanjangan,
                akhir_peminjaman: prevAkhir,
              }
            : r,
        ),
      );
  ...
  async function rejectExtension(row: AdminLoanRow) {
    ...
    setRows((current) =>
      current.map((r) =>
        r.id === row.id ? { ...r, status_perpanjangan: "DITOLAK" } : r,
      ),
    );
    ...
    try {
      ...
      setRows((current) =>
        current.map((r) =>
          r.id === row.id
            ? { ...r, status_perpanjangan: prevStatusPerpanjangan }
            : r,
        ),
      );
                  ...
                  <tbody className="divide-y">
                    {rows.map((r) => {
                      ...
                      const extStatus = String(
                        r.status_perpanjangan ?? "",
                      ).toUpperCase();
                      const extLabel = extensionLabel(extStatus);

                      const hasPendingExtension =
                        extStatus === "DIAJUKAN" &&
                        Boolean(r.akhir_perpanjangan);
                      ...
                      return (
                        <tr key={r.id} className="hover:bg-slate-50">
                                ...
                                <div className="text-xs text-slate-600">
                                  ...
                                  {r.akhir_perpanjangan ? (
                                    <>
                                      ...
                                      <span>
                                        hingga{" "}
                                        {formatDate(r.akhir_perpanjangan)}
                                      </span>
                                    </>
                                  ) : null}
                                </div>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            ) : hasPendingExtension && !isReturned ? (
                              <div className="flex items-center gap-2">
                                <button
                                  type="button"
                                  onClick={() => approveExtension(r)}
                                  disabled={mutatingId === r.id}
                                  className="inline-flex h-9 items-center rounded-lg bg-green-600 px-3 text-sm font-medium text-white hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-60"
                                >
                                  Setujui Perpanjangan
                                </button>
                                <button
                                  type="button"
                                  onClick={() => rejectExtension(r)}
                                  disabled={mutatingId === r.id}
                                  className="inline-flex h-9 items-center rounded-lg bg-red-600 px-3 text-sm font-medium text-white hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60"
                                >
                                  Tolak Perpanjangan
