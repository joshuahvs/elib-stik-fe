...
export default function KoleksiDetailPage() {
  ...
  useEffect(() => {
    ...
    fetchBukuById(params.id)
      .then((data) => { setBook(data); if (!data) setError(true); })
      .catch(() => setError(true))
  ...
  useEffect(() => {
    if (!token || !params.id) { setDigitalMeta(null); return; }
    const authToken = token;
    const stableId = params.id;
    let cancelled = false;
    async function run() {
      try {
        const res = await fetch(`${API_URL}/books/${encodeURIComponent(stableId)}`, {
          method: "GET",
          headers: { Authorization: `Bearer ${authToken}`, Accept: "application/json" },
          cache: "no-store",
        });
        const data = await res.json().catch(() => null);
        if (!res.ok) { if (!cancelled) setDigitalMeta(null); return; }
        if (!cancelled) setDigitalMeta((data ?? null) as BookDigitalMeta | null);
      } catch { if (!cancelled) setDigitalMeta(null); }
    }
    run();
...
const loadReviews = async () => {
  ...
  const digitalId = useMemo(() => (params.id ? String(params.id) : null), [params.id]);
  ...
  async function handleOpenDigital() {
    ...
    try {
      setOpening(true); setPageError(null);
      const { signedUrl } = await fetchDigitalSignedUrl({ token: t, bookId: digitalId });
      openInNewTab(signedUrl);
    } catch (e) { setPageError(getErrorMessage(e, "Buku Digital Belum Tersedia")); }
  ...
  function openLoanModal() {
    const t = window.localStorage.getItem("token");
    if (!t) { window.location.href = "/auth/login"; return; }
    if (!numericBukuId) { setLoanError("ID buku tidak valid."); setLoanSuccess(null); return; }
    if (!tanggalPinjam) setTanggalPinjam(todayYmd);
    if (!Number.isFinite(periodeHari) || periodeHari <= 0) setPeriodeHari(7);
    setLoanError(null); setLoanSuccess(null); setLoanModalOpen(true);
  }

  async function handleAjukanPeminjaman() {
    const t = window.localStorage.getItem("token");
    if (!t) { window.location.href = "/auth/login"; return; }
    if (!numericBukuId) { setLoanError("ID buku tidak valid."); return; }
    if (!tanggalPinjam || !akhirPinjam) { setLoanError("Pilih tanggal dan periode peminjaman."); return; }
    if (tanggalPinjam < formatLocalYmd(new Date())) { setLoanError("Tanggal tidak boleh di masa lalu."); return; }
    try {
      setSubmittingLoan(true); setLoanError(null); setLoanSuccess(null);
      await ajukanPeminjamanBuku({ token: t, bukuId: numericBukuId, tanggal_peminjaman: tanggalPinjam, akhir_peminjaman: akhirPinjam });
      setLoanSuccess("Pengajuan peminjaman berhasil dikirim.");
      setLoanModalOpen(false);
    } catch (e) { setLoanError(getErrorMessage(e, "Gagal mengajukan peminjaman")); }
    finally { setSubmittingLoan(false); }
  }
  ...
  async function handleSubmitReview() {
    const t = localStorage.getItem("token");
    if (!t) { window.location.href = "/auth/login"; return; }
    if (!commentInput.trim()) { setReviewError("Komentar tidak boleh kosong."); return; }
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ buku_id: Number(params.id), rating: ratingInput, comment: commentInput }),
      });
      ...
      setCommentInput("");
      await loadReviews();
    } catch (e) { setReviewError(e instanceof Error ? e.message : "Gagal menyimpan ulasan."); }
  ...
  function openAddReviewModal() {
    const t = localStorage.getItem("token");
    if (!t) { window.location.href = "/auth/login"; return; }
    setEditingReview(null); setRatingInput(5); setCommentInput(""); setReviewError(null);
    setReviewModalOpen(true);
  }

  function openEditReviewModal(r: Review) {
    setEditingReview(r); setRatingInput(r.rating); setCommentInput(r.comment); setReviewError(null);
    setReviewModalOpen(true);
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        ...
        {loanSuccess && (
          <div className="mb-6 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">{loanSuccess}</div>
        )}
            ...
            <div className="w-full shrink-0 md:w-72">
              <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl shadow-xl ring-1 ring-black/5">
                <img
                  ...
                  onError={(e) => { const img = e.currentTarget; img.onerror = null; img.src = FALLBACK_COVER; }}
              ...
              {reviews.length > 0 && (
                <div className="mt-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-center">
                  <p className="text-[11px] font-medium uppercase tracking-widest text-slate-400">Rating</p>
                  <p className="mt-0.5 text-2xl font-bold text-slate-800">{avgRating}</p>
                  <div className="mt-1 flex justify-center"><StarRating rating={Math.round(Number(avgRating))} /></div>
                  <p className="mt-0.5 text-xs text-slate-400">{reviews.length} ulasan</p>
                </div>
              )}
            </div>
            ...
            <div className="flex-1">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-3">
                ...
                {hasDigital ? (
                  ...
                  <button type="button" disabled className="inline-flex h-10 cursor-not-allowed items-center rounded-lg bg-[#6b3a22] px-4 text-sm font-semibold text-white opacity-60 shadow-sm">
        ...
        <section className="mt-14">
          ...
          {reviews.length > 0 && (
            <div className="mt-5 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5 sm:flex-row sm:items-center">
              <div className="flex flex-col items-center sm:w-28">
                <span className="text-5xl font-bold text-slate-800">{avgRating}</span>
                <StarRating rating={Math.round(Number(avgRating))} />
                <span className="mt-1 text-xs text-slate-400">{reviews.length} ulasan</span>
              </div>
                ...
                {[5, 4, 3, 2, 1].map((star) => {
                  ...
                  return (
                    <div key={star} className="flex items-center gap-2 text-sm">
                      ...
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 shrink-0 text-orange-400" viewBox="0 0 20 20" fill="currentColor">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
          ...
          {reviewLoading ? (
            ...
            <div className="mt-6 flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white py-12 text-slate-400">
              ...
              <p className="mt-0.5 text-xs">Jadilah yang pertama memberikan ulasan!</p>
            </div>
          ) : (
              ...
              {reviews.map((r) => (
                <div key={r.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="mt-2.5">
                    ...
                    <p className="mt-1.5 text-sm leading-relaxed text-slate-600">{r.comment}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {loanModalOpen && (
          <div
            ...
            onMouseDown={(e) => { if (e.target === e.currentTarget) setLoanModalOpen(false); }}
          >
            <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-lg">
              <div className="text-lg font-semibold text-slate-900">Ajukan Peminjaman</div>
              <div className="mt-4 grid gap-4">
                <div>
                  <label className="mb-2 block text-sm text-slate-700">Tanggal Peminjaman</label>
                  <input
                    type="date" min={todayYmd} value={tanggalPinjam}
                    onChange={(e) => {
                      const next = e.target.value;
                      setTanggalPinjam(next);
                      if (next && next < todayYmd) { setLoanError("Tanggal tidak boleh di masa lalu."); setLoanSuccess(null); }
                      else if (loanError) setLoanError(null);
                    }}
                    className="h-11 w-full rounded-xl border border-slate-300 px-3 text-black"
                  />
                </div>
                <div>
                  <label className="mb-2 block text-sm text-slate-700">Periode (hari)</label>
                  <input
                    type="number" min={1} value={periodeHari}
                    onChange={(e) => setPeriodeHari(Number(e.target.value))}
                    className="h-11 w-full rounded-xl border border-slate-300 px-3 text-black"
                  />
                </div>
                <div className="text-sm text-slate-600">Akhir Peminjaman: {akhirPinjam || "-"}</div>
              </div>
              <div className="mt-6 flex justify-end gap-3">
                <button type="button" onClick={() => setLoanModalOpen(false)} disabled={submittingLoan} className="h-10 rounded-xl border border-slate-300 bg-white px-4 font-medium text-slate-900 hover:bg-slate-50">Batal</button>
                <button type="button" onClick={handleAjukanPeminjaman} disabled={submittingLoan || !numericBukuId} className="h-10 rounded-xl bg-[#6b3a22] px-4 font-medium text-white hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-60">
        ...
        {reviewModalOpen && (
          <div
            className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-6"
            role="dialog" aria-modal="true"
            onMouseDown={(e) => { if (e.target === e.currentTarget) setReviewModalOpen(false); }}
          >
            <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-lg">
              ...
              {reviewError && (
                <div className="mt-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{reviewError}</div>
              )}
              ...
              <div className="mt-6 flex justify-end gap-3">
                <button
                  type="button"
                  onClick={() => { setReviewModalOpen(false); setEditingReview(null); setReviewError(null); }}
        ...
        {deleteConfirmId !== null && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 px-6" role="dialog" aria-modal="true">
            <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-lg">
              ...
              <p className="mt-2 text-sm text-slate-500">Ulasan yang dihapus tidak dapat dikembalikan.</p>
              <div className="mt-6 flex justify-end gap-3">
                <button onClick={() => setDeleteConfirmId(null)} className="h-10 rounded-xl border border-slate-300 bg-white px-4 font-medium text-slate-900 hover:bg-slate-50">Batal</button>
                <button onClick={() => handleDeleteReview(deleteConfirmId)} className="h-10 rounded-xl bg-red-500 px-4 font-medium text-white hover:opacity-90">Hapus</button>
              </div>
            </div>
          </div>
        )}
